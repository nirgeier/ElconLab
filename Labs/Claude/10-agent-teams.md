...
## Demo A - fan-out research team

Goal: run parallel discovery across independent areas.

Prompt:

```text
Run a fan-out analysis using subagents.
1) Analyze auth module for token/session logic.
2) Analyze billing module for retry/idempotency logic.
3) Analyze notifications module for delivery failure handling.
Return a merged summary with file paths and top 5 risks.
```

Success criteria:

- Each area is analyzed independently.
- Final report is merged, concise, and path-specific.

---

## Demo B - implement then verify pattern

Goal: enforce a doer + checker workflow.

Prompt:

```text
Use implementer to add a default HTTP timeout of 15s where missing.
Then use reviewer to check regressions and missing tests.
Return changed files, risk findings, and recommended test commands.
```

Success criteria:

- Reviewer runs after implementation.
- Risk findings are severity-ordered.
- Output includes concrete test commands.
