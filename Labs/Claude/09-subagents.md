...
---
  ...
  ## Demo - create and run your first subagent flow

  Use this mini demo in any small repo to see delegation in action.
  ...
  ---
  name: researcher
  description: Explore code and report exact file paths.
  tools: Read, Glob, Grep
  model: sonnet
  ---
  You are a read-only researcher. Return findings with exact paths.
  ```

  2. Start Claude and prompt:

  ```text
  Use the researcher subagent to find where API timeout values are configured.
  Return the top 3 files and suggested change location.
  ```

  3. Validate output quality:

  - Are paths exact and actionable?
  - Are assumptions clearly labeled?
  - Did the agent avoid editing files as expected?
